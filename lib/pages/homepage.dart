import 'package:app_contas_v1/controllers/db_helper.dart';
import 'package:app_contas_v1/pages/add_transaction.dart';
import 'package:app_contas_v1/pages/models/transaction.dart';
import 'package:app_contas_v1/pages/settings.dart';
import 'package:app_contas_v1/pages/widgets/confirm_dialog.dart';
import 'package:app_contas_v1/pages/widgets/info_snackbar.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  List<FlSpot> getPlotPoints(List<TransactionModel> entireData) {
    ...
    for (TransactionModel item in entireData) {
      if (item.date.month == today.month && item.type == "Crédito") {
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasData) {
            if (snapshot.data!.isEmpty) {
              return Center(
                child: Text(
                  "Você não adicionou nenhuma movimentação",
                  style: TextStyle(
                    fontSize: 24.0,
                  ),
                ),
              ...
              children: [
                      ...
                      Row(
                        children: [
                          ...
                          SizedBox(
                            width: 200.0,
                            child: Text(
                              "Bem-vindo ${preferences.getString('name')}",
                              style: TextStyle(
                                fontSize: 24.0,
                ...
                dataSet.isEmpty || dataSet.length < 2
                    ? Container(
                        ...
                        child: Text(
                          "Não há movimentações suficientes",
                          style: TextStyle(
                            fontSize: 20.0,
                          ),
                        ),
    ...
    return InkWell(
      ...
      onLongPress: () async {
        bool? answer = await showConfirmDialog(
          context,
          "Aviso",
          "O dado será apagado. Ação irreversível. Deseja continuar?",
        );
